...
describe('SwapRouter', () => {
  ...
  beforeEach('load fixture', async () => {
    ;({ router, weth9, factory, tokens, nft } = await swapRouterFixture(wallets))
  ...
  describe('swaps', () => {
    ...
    describe('#exactInput', () => {
      async function exactInput(
        ...
        await expect((router as any).connect(trader).exactInput(params, { value })).to.be.revertedWith(
          'Too little received'
        )
        params.amountOutMinimum -= 1
        ...
        describe('WETH9', () => {
          ...
          it('WETH9 -> 0', async () => {
            ...
            await expect(exactInput([weth9.address, tokens[0].address]))
              ...
              .withArgs('0x0000000000000000000000000000000000000000', router.address, 3)
          ...
          it('WETH9 -> 0 -> 1', async () => {
            ...
            await expect(exactInput([weth9.address, tokens[0].address, tokens[1].address], 5))
              .to.emit(weth9, 'Transfer')
              .withArgs('0x0000000000000000000000000000000000000000', router.address, 5)
        ...
        describe('WETH9', () => {
          ...
          it('0 -> WETH9', async () => {
            ...
            await expect(exactInput([tokens[0].address, weth9.address]))
              .to.emit(weth9, 'Transfer')
              .withArgs(router.address, '0x0000000000000000000000000000000000000000', 1)
    ...
    describe('#exactOutput', () => {
      async function exactOutput(
        ...
        await expect((router as any).connect(trader).exactOutput(params, { value })).to.be.revertedWith(
          'Too much requested'
        )
        params.amountInMaximum += 1
